...
class Graph:
  ...
  def explore(self):
    ...
    #will let you explore the graph appropriately
    current_room = "entrance"
    path_total=0
    print("\nStarting off at the {0}\n".format(current_room))
    #print("Edges ",self.graph_dict[current_room].get_edges())
    while(current_room != "treasure room"):
      node=self.graph_dict[current_room] #GETS THE entrance edges, rooms connected to entrance
      for connected_room, weight in node.edges.items():
        key=connected_room[0] #hints to decide which room we go into
        print("enter {0} for {1}: {2} cost".format(key, connected_room, weight))
        #list all the first letters of each room
      valid_choices=[room[0] for room in node.edges.keys() ]
      print("\nYou have accumulated: {0} cost".format(path_total))
    #let the user-explorer make the choice where they go
      choice=input("\nWhich room do you move to? ")
      if(choice not in valid_choices):#check if choice is correct
        print("please select from these letters: {0}".format(valid_choices))
      else:
        for room in node.edges.keys():
          if(room.startswith(choice)):
            current_room=room #update room
            path_total+= node.edges[current_room] #update path taken
            break
        print("\n*** You have chosen: {0} ***\n".format(current_room))
